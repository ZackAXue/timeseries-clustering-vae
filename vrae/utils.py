...
def plot_clustering(
    z_run, labels, engine="plotly", download=False, folder_name="clustering"
):
    """
    ...
    def plot_clustering_plotly(z_run, labels):
        ...
        z_run_tsne = TSNE(
            perplexity=80, min_grad_norm=1e-12, n_iter=3000
        ).fit_transform(z_run)
    ...
    def plot_clustering_matplotlib(z_run, labels, download, folder_name):
        ...
        z_run_tsne = TSNE(
            perplexity=18, min_grad_norm=1e-12, n_iter=3000
        ).fit_transform(z_run)

        plt.scatter(
            z_run_pca[:, 0], z_run_pca[:, 1], c=colors, marker="*", linewidths=0
        )
        ...
        plt.scatter(
            z_run_tsne[:, 0], z_run_tsne[:, 1], c=colors, marker="*", linewidths=0
        )
